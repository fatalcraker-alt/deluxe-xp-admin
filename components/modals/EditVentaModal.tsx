...
interface Props {
  ...
  onClose: () => void;
  onSaved: (updated: Venta) => void;
}

export default function EditVentaModal({ venta, fragancias, onClose, onSaved }: Props) {
  const supabase = createClient();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [form, setForm] = useState({
    fecha: venta.fecha,
    fragancia_id: venta.fragancia_id ?? "",
    tipo: venta.tipo as "5ml" | "10ml" | "completo" | "promo",
    cantidad: String(venta.cantidad),
    precio_unitario: String(venta.precio_unitario),
    canal: venta.canal as "whatsapp" | "presencial" | "otro",
    notas: venta.notas ?? "",
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full sm:max-w-md bg-surface border border-border p-6 space-y-5 sm:m-4">
        <div className="flex items-center justify-between">
          <h2 className="font-serif text-xl text-brand-white tracking-wide">Editar Venta</h2>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="col-span-2">
              <label className="flex items-center gap-3 cursor-pointer group">
                <div
                  onClick={() => set("cobrado", !form.cobrado)}
                  className={`w-10 h-5 rounded-full transition-colors relative flex-shrink-0 ${form.cobrado ? "bg-emerald-500" : "bg-border"}`}
                >
                  <span className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white transition-transform ${form.cobrado ? "translate-x-5" : "translate-x-0"}`} />
                </div>
                <span className={`font-sans text-xs tracking-widest uppercase ${form.cobrado ? "text-emerald-400" : "text-alert-red"}`}>
                  {form.cobrado ? "Cobrado" : "Pendiente de cobro"}
                </span>
              </label>
            </div>
          </div>
