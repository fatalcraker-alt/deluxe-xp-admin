...
export default function EditFraganciaModal({ fragancia, onClose }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    ml_original: String(fragancia.ml_original),
    ml_actuales: String(fragancia.ml_actuales),
    costo_adquisicion: String(fragancia.costo_adquisicion ?? ""),
    precio_ml: String(fragancia.precio_ml ?? ""),
  });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        ml_actuales: parseFloat(form.ml_actuales),
  ...
  async function handleDesactivar() {
    if (!confirm(`¿Desactivar "${fragancia.nombre}"? Se ocultará del inventario.`)) return;
    await supabase.from("fragancias").update({ activo: false }).eq("id", fragancia.id);
    onClose();
  }
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full sm:max-w-md bg-surface border border-border p-6 space-y-5 sm:m-4">
        <div className="flex items-center justify-between">
          <h2 className="font-serif text-xl text-brand-white tracking-wide">Editar Fragancia</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1">
              ...
              <input required type="number" min="0" value={form.ml_actuales} onChange={(e) => set("ml_actuales", e.target.value)}
                className="w-full bg-brand-black border border-border text-brand-white font-sans text-sm px-3 py-2 outline-none focus:border-brand-gray transition-colors" />
            </div>
            <div className="space-y-1">
          ...
          <div className="flex gap-2 pt-1">
            <button type="button" onClick={handleDesactivar}
              className="px-3 border border-alert-red/40 text-alert-red/70 font-sans text-[10px] tracking-widest uppercase py-2.5 hover:border-alert-red hover:text-alert-red transition-colors">
              Desactivar
            </button>
